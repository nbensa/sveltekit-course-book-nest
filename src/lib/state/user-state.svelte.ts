...
export class UserState {
  ...
  async updateBook(bookId, updateObject) {
    if (!this.supabase) {
      return;
    }
  ...
  async uploadBookCover(file, bookId) {
    ...
    const { erro, uploadError } = await this.supabase.storage
      .from("book-covers")
  ...
  async addBooksToLibrary(booksToAdd) {
  ...
  async updateAccountData(email, userName) {
    if (!this.session) {
      return;
    }
